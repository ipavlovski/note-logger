 ...
 * ```
 */
export default function PDF({ uri, uris }: { uri: string, uris: string[] }) {
...
function ProgressBar({ uris }: { uris: string[] }) {
  const { classes } = useStyles()

  return (
    <div className={classes.bar}>
      {uris.map((uri, ind) => (
        <Point uri={uri} key={ind} />
      ))}
    </div>
  )
}

function Point({ uri }: { uri: string }) {
  const { classes } = useStyles()
  ...
  const clickHandler = () => {
    console.log(`clicked on uri: ${uri} @ ${page} / ${itemCount}`)
